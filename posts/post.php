...
if ($result_post->num_rows > 0) { 
    ...
    while($row = $result_post->fetch_assoc()) { 
        ...
        echo '<a href = "http://localhost/web4/posts/posts.php" class = "back">⭠ Назад</a>';
...
if (isset($_POST['submit_comment'])) { 
    $author_name = isset($_SESSION['us_name']) ? $conn->real_escape_string($_SESSION['us_name']) : ''; 
...
if ($result_comments->num_rows > 0) { 
    ...
    while($row = $result_comments->fetch_assoc()) { 
        echo "<p><b>" . htmlspecialchars($row['author_name']) . "</b> (" . $row['created_at'] . ") : " . nl2br(htmlspecialchars($row['content'])) . " </p>"; 
